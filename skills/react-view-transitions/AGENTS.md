...
Read [SKILL.md](SKILL.md) first. It links the current React and Next.js documentation and routes deeper work to focused references.
...
- Keep route enter/exit VTs in pages unless the layout itself is meant to animate.
- Test prefetched and suspended routes separately.
- For reveal-only crossfades, keep `Crossfade` wrapper-free and add a DOM host outside `Suspense` at affected navigable call sites.
- Keep optimistic feedback separate from the committed element that owns a shared transition name.
...
- [Supplemental CSS recipes](references/css-recipes.md)
